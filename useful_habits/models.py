...
class Habit(models.Model):
    ...
    owner = models.ForeignKey(
        ...
        verbose_name='владелец', on_delete=models.DO_NOTHING
    )
    place = models.CharField(
        max_length=150, verbose_name='место выполнения'
    )
    time = models.CharField(
        max_length=150, verbose_name='время выполнения'
    )
    action = models.TextField(
        max_length=700, verbose_name='действие'
    )
    usefulness = models.BooleanField(
        default=False, verbose_name='полезность'
    )
    pleasantness = models.BooleanField(
        default=False, verbose_name='приятность'
    )
    connectivity = models.ManyToManyField(
        'self', symmetrical=False, default=None,
        verbose_name='связанные привычки'
    )
    frequency = models.CharField(
        choices=FREQUENCY_CHOISES, max_length=150,
